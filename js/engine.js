/* ============================================================
   MI RED DE APOYO — Motor del juego (prototipo)
   ============================================================ */
...
/* ---------------- REGISTRO DE PARTICIPANTE (datos mínimos, opcionales) ---------------- */
function validateRegistration() {
  const age = $("#regAge").value;
  const name = $("#regName").value.trim();
  const email = $("#regEmail").value.trim();
  const emailOk = email === "" || /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email);
  const contactProvided = name !== "" || email !== "";
  $("#regConsentWrap").classList.toggle("hidden", !contactProvided);
  const consentOk = !contactProvided || $("#regConsent").checked;
  const ok = age !== "" && emailOk && consentOk;
...
$("#regSkipBtn").addEventListener("click", () => {
  state.player = { name: "", age: "", email: "" };
  renderCharacterSelect();
  showScreen("screen-select");
});
...
function renderNode() {
  ...
  if (state.speechAutoplay && node.text) speakText(node.text, node.speaker);
...
function computeEnding() {
  ...
  const ending = ENDINGS.find((e) => score >= e.min);
